...
A API de Backoffice expõe operações administrativas de **consulta** sobre os dados de consentimentos, vínculos e pagamentos do Módulo de Iniciação de Pagamentos. É destinada a equipes de suporte, operações e backoffice, **não substituindo** as interfaces voltadas ao usuário final ou ao app cliente.
...
| `tppId` | UUID | Identificador da TPP (quando o Módulo de Iniciação de Pagamentos atende múltiplas TPPs) |
...
- **Relatórios operacionais:** combinar com `tppId` quando o Módulo de Iniciação de Pagamentos atende múltiplas marcas/instituições.
